public static class CharacterCreator
{
  ...
  {
    Interface.WriteLine("Wie heißt Du, mutiger Held?", ConsoleColor.Yellow);
    string? playerName = Console.ReadLine();
    if (playerName == null || string.IsNullOrWhiteSpace(playerName))
    {
      playerName = "Bimbatz";
      Interface.WriteLine("Du hast nichts eingegeben! Wir nennen Dich einfach " + playerName + ".");
    }
    Interface.WriteLine("Willkommen, " + playerName + "!");
    ...
    Interface.WriteLine($"Du bist {player.Name}, ein {player.Origin.Race} aus dem {player.Origin.Homeland}. Deine Profession ist {player.Class.Name}.");
    return player;
  }
}